# evaluate retrieval performance on Arguana, Quora, NFCorpus, and SciFact
...
model = SelfStrAE(25001, 256, 16, 0.1, device).to(device)

# load from checkpoint
print('Loading model from:', args.checkpoint_path)
checkpoint = torch.load(args.checkpoint_path)
model.load_state_dict(checkpoint['model'])
model = DRES(StrAE_DE(model), batch_size=128)
